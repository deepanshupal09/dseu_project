        ...
        <>
            <div className="relative">
                <div className="flex items-center fixed max-sm:top-2 max-sm:right-7 top-8 right-20 z-30">
                    <button className="relative w-10 h-10" onClick={() => setIsOpen(!isOpen)} aria-label="Menu">
                        <div className={`absolute top-1/2 left-1/2 w-8 h-[2px] bg-gray-300 transition-transform duration-300 ease-in-out ${isOpen ? "rotate-45" : "-translate-y-2"} transform origin-center`}></div>
                        <div className={`absolute top-1/2 left-1/2 w-8 h-[2px] bg-gray-300 transition-opacity duration-300 ease-in-out ${isOpen ? "opacity-0" : ""}`}></div>
                        <div className={`absolute top-1/2 left-1/2 w-8 h-[2px] bg-gray-300 transition-transform duration-300 ease-in-out ${isOpen ? "-rotate-45" : "translate-y-2"} transform origin-center`}></div>
                    </button>
                </div>
                ...
                {/* <div className={`fixed top-0 right-0 z-20 w-screen h-screen bg-green-400 transition-transform duration-[400ms] ease-in-out ${isOpen ? "translate-y-0" : "-translate-y-full"}`}></div>
                ...
                <div className={`fixed top-0 right-0 z-20 w-screen h-screen bg-white transition-transform duration-[600ms] ease-in-out ${isOpen ? "translate-y-0" : "-translate-y-full"}`}>
                    <div className={`h-[2px] fixed top-20 w-full bg-gray-300 transition-all duration-700 ease-in-out ${isOpen ? "w-full" : "w-0"}`}></div>
                    <div className={`h-[2px] fixed bottom-20 w-full bg-gray-300 transition-all duration-700 ease-in-out ${isOpen ? "w-full" : "w-0"}`}></div>
                    <div className={`fixed top-20 bottom-20 w-[2px] right-1/2 bg-gray-300 transition-all duration-1000 ease-in-out ${startBottomAnimation ? "h-[calc(100vh-160px)]" : "h-0"}`}></div>
                    <div className={`fixed top-20  `}></div>
                </div>
            </div>
            <div className=" ">
