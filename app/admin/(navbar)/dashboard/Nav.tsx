...
export default function Nav() {
  ...
  const options = ["Dashboard", "Registration Chart", "Admit Card", "Edit Student Details","Query", "Exam Control"];
  useEffect(() => {
          ...
          <div>
            <div className="w-[250px] text-lg font-normal px-8 py-48 space-y-10 h-full  shadow-2xl shadow-slate-200 bg-white absolute border-2 left-0 sm:block hidden">
              {options.map((option, index) => {
                const path = `/admin/${option
                  .toLowerCase()
                  .replace(/\s+/g, "-")}`;
                return (
                  <div
                    key={index}
                    className={`cursor-pointer hover:text-dseublue  ${
                      path === activeLink ? "text-dseublue " : ""
                    }`}
                  >
