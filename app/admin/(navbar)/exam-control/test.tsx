...
export default function Registration() {
 ...
 const [campusStatus, setCampusStatus] = useState<Record<string,boolean>>({});
 ...
 const handleSemesterCheckboxChange = (campus: string, program: string, semester: string, event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  setSelectedSemester((prev) => {
   const key = `${campus}-${program}`;
   const updatedSemesters = value
    ? [...(prev[key] || []), semester]
    : (prev[key] || []).filter((s) => s !== semester);
   return { ...prev, [key]: updatedSemesters };
  });
 };
 ...
 const handleConfirmFilteredAction = () => {
  ...
  setSnackbarMessage(
   openCloseAction === "open"
    ? "Exam registrations opened for filtered selection"
    : "Exam registrations closed for filtered selection"
  );
 ...
 return (
  <>
   ...
   <Dialog open={openCloseModal} onClose={handleCloseModal}>
    ...
    <DialogContent>
     {`Are you sure you want to ${openCloseAction} exam registrations for selected campuses, programs, and semesters?`}
    </DialogContent>
    <DialogActions>
