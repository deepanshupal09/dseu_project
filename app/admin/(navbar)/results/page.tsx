...
export default function Registration() {
  ...
  const processStudentData = (data: any) => {
    return data.map((student: any) => {
      ...
      const { campus, program, program_type, semester, year_of_admission, abc_id, father, mother, guardian, name } =
        personalDetails;
      const { rollno, sgpa_result, sgpa_grade, Credits_earned } = markSheet;

      const aggregateMarks = markSheet.aggregate_marks
        ? markSheet.aggregate_marks.map((mark: any) => ({
      ...
      const internalMarks = markSheet.internal_marks
        ? markSheet.internal_marks.map((mark: any) => ({
            course_code: mark.course_code,
            grade: mark.grade,
          }))
        : [];

      const externalMarks = markSheet.external_marks
        ? markSheet.external_marks.map((mark: any) => ({
            course_code: mark.course_code,
            grade: mark.grade,
          }))
        : [];

      const bridgeMarks = markSheet.bridge_marks
        ? markSheet.bridge_marks.map((mark: any) => ({
            course_code: mark.course_code,
            course_name: mark.course_name,
            grade: mark.grade,
            grade_point: mark.grade_point,
  ...
  const renderTableRows = (student: any) => {
        ...
        <tbody className="font-semibold">
          <tr>
            <td colSpan={9} className="text-center">
              No aggregate marks found for this student
            </td>
          </tr>
        </tbody>
      );
    ...
    const normalCourseRows = markSheet.aggregateMarks.map((aggregateMark: any, index: any) => {
      const internalMark =
        (markSheet.internalMarks || []).find((mark: any) => mark.course_code === aggregateMark.course_code) || {};
      const externalMark =
        (markSheet.externalMarks || []).find((mark: any) => mark.course_code === aggregateMark.course_code) || {};
      return (
    ...
    const bridgeCourseRows =
      markSheet.bridgeMarks && markSheet.bridgeMarks.length > 0
        ? markSheet.bridgeMarks.map((bridgeMark: any, index: any) => (
            <tr key={`${student.abc_id}-bridge-${index}`}>
              <td className="px-2 py-3 border border-black text-center text-xs font-medium">
                {markSheet.aggregateMarks.length + index + 1}
              </td>
    ...
    return <tbody className="font-semibold">{allRows}</tbody>;
  };

  const handlePrint = () => {
    const printWindow = window.open("", "_blank");
    if (printWindow) {
      const allMarksheets = filteredData
        .map(
          (student, index) => `
        <div class="print-page-break">
          ${
            student.personalDetails
              ? `<div class="mx-8 my-2 font-bold"><h4>ABC ID: ${student.personalDetails.abc_id}</h4></div>`
              : ""
          }
          ...
          <table class="w-11/12 mx-auto leading-normal mt-5 font-bold uppercase tracking-wider font-roboto sm:overflow-x-hidden">
            ...
            <tbody class="bg-white">
              <tr class="border border-black">
                ...
                <td class="px-6 py-4 text-center border border-black whitespace-nowrap">${student.markSheet.sgpa_result.toPrecision(
                  3
                )}</td>
          ...
          {isApplied && (!filteredData || filteredData.length === 0) ? (
            ...
            <>
              ...
              <div className="w-full overflow-x-auto">
                <div
                  ref={componentRef}
                  className={`py-1 px-2 rounded sm:mx-auto mt-6 relative w-[1400px] sm:overflow-x-hidden ${
                    mode === "print" ? "print-mode" : ""
                  }`}
                >
                  {(mode === "view"
                    ? filteredData.slice(currentPage * studentsPerPage, (currentPage + 1) * studentsPerPage)
                    : filteredData
                  ).map((student, index) => (
                    <div key={index} className="print-page-break">
                      ...
                      <div className="text-center flex flex-col mx-auto">
                        ...
                        <div className="text-lg font-bold font-serif mb-4">
                          {student && student?.personalDetails.program}-Batch{" "}
                          <span className="font-sans">{student?.personalDetails.year_of_admission}</span>
                        </div>
                      </div>
                      <div className="border border-solid py-16">
                        <div className="text-center flex flex-col my-2 mr-5 px-14 sm:">{renderParentInfo(student)}</div>
                ...
                {mode === "view" && filteredData.length > studentsPerPage && (
                  <div className="flex justify-center mt-4">
                    <button onClick={handlePrevPage} disabled={currentPage === 0} className="px-4 py-2 mx-2 bg-gray-300 rounded">
                      Previous
                    </button>
                    <span>
