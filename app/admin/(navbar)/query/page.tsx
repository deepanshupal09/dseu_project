...
import { fetchCoursesBySemester, fetchExamRegistrationByCourseCode } from "@/app/actions/api";
import { getAuthAdmin } from "@/app/actions/cookie";
import { parseJwt } from "@/app/actions/utils";
import { DataGrid, GridColDef, GridCsvExportMenuItem, GridCsvExportOptions, GridToolbarContainer, GridToolbarContainerProps, GridToolbarExportContainer } from "@mui/x-data-grid";
...
export default function Registration() {
    ...
    const handleApplyFilters = async () => {
        if (selectedCourse !== "") {
            ...
            if (course_code) {
                try {
                    const res = await fetchExamRegistrationByCourseCode(token, selectedCampus, course_code);
                    const formattedStudentList = res.map((student: Student, index: number) => ({ ...student, id: index + 1 }));
    ...
    return (
        <>
            ...
            <div className="py-2 px-4 rounded shadow absolute top-[200px] sm:left-[250px] left-0 right-0  mx-2 sm:mx-12 mt-6">
                ...
                <div className="my-10" id="datagrid-container" style={{ height: 670, width: "100%" }}>
                    <DataGrid
