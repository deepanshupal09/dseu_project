...
import { fetchCourseDetailsByCourseCode, fetchCoursesBySemester, fetchExamRegistrationByProgramAndSemester } from "@/app/actions/api";
...
export default function Registration() {
    ...
    useEffect(() => {
        if (user && selectedProgram !== "" && selectedSemester !== "") {
            fetchCourseDetailsByCourseCode(token, { coursecode: courseCodes, campus: selectedCampus, program: selectedProgram })
    ...
    const handleData = async () => {
        try {
            if (token) {
                const data: any = await fetchExamRegistrationByProgramAndSemester(token as string, selectedCampus, selectedProgramCategory, selectedProgram, selectedSemester);
              ...
              Apply
                ...
                <div className="py-2 px-2 rounded shadow sm:mx-8 mt-6">
                    ...
                    <div ref={containerRef} className=" h-auto">
                        ...
                        <div ref={frontSheetRef} className="front-sheet hidden overflow-y-auto h-auto">
                                                ...
                                                <TableRow key={index} sx={{}}>
                                                    ...
                                                    <TableCell style={{ border: "1px solid black" }}>
                                                        <div className="flex flex-wrap p-2">
                                                            {student.course_codes.slice(0, 6).map((code, codeIndex) => (
                                                                <div
                                                                    className="cell"
                                                                    key={codeIndex}
                                                                    style={{
                                                        ...
                                                        <div className="flex flex-wrap">
                                                            {student.course_codes.slice(6).map((code, codeIndex) => (
                                                                <div
                                                                    key={codeIndex}
                                                                    className="cell"
                                                                    style={{
