...
import { Dialog, DialogActions, DialogContent, DialogTitle, Menu, Snackbar } from "@mui/material";
...
export default function MarksTable({ students, maxMarks }: { students: StudentType[]; maxMarks: number }) {
    ...
    async function handleApplyChanges() {
        ...
        studentList.map((student) => {
            if (isNaN(parseInt(student.marks)) && student.marks.trim() !== "A") {
                ...
                errors.push({ ...student, error: `Marks can not be larger than ${maxMarks}!` });
    ...
    return (
        <div className="space-y-5">
            ...
            <div className={`w-full h-[25vh] bordered rounded-3xl space-y-3 text-sm font-normal flex flex-col justify-center items-center ${dragActive ? "bg-slate-100" : ""}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
                {!fileData && (
                    <>
            ...
            <div>
                <div>
                                            ...
                                            {columns.map((column) => {
                                                ...
                                                return (
                                                    <TableCell key={column.id} align={column.align}>
                                                        {column.id === "marks" ? <TextField value={value} onChange={(e) => handleMarksChange(e, rowIndex)} size="small" /> : column.format && typeof value === "number" ? column.format(value) : value}
                                                    </TableCell>
                                                );
                                            })}
                                        </TableRow>
                    ...
                    <TablePagination rowsPerPageOptions={[25, 50, 100]} component="div" count={studentList.length} rowsPerPage={rowsPerPage} page={page} onPageChange={handleChangePage} onRowsPerPageChange={handleChangeRowsPerPage} />
                </div>
            </div>
                ...
                <DialogContent>
                    ...
                    {errors.map((error, index) => (
                        <Typography key={index}>
                            {`[${index}]: `} Error at <span className="font-bold">S.No {error.sno}</span> ,Name <span className="font-bold"> {error.name}</span>, <span className="font-bold">{error.error}</span>
                        </Typography>
                    ))}
                </DialogContent>
                <DialogActions>
