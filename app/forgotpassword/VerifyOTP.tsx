...
export default function VerifyOTP({ rollno, setStep, otp, setOtp }: { rollno: string; setStep: Function; otp: string; setOtp: Function }) {
    ...
    async function handleVerifyOtp() {
        ...
        // }
        setLoading(true);
        verifyOtpAndPassword(rollno, otp)
            .then((res) => {
                if (res.status === 200) {
                    setStep(3);
                    setLoading(false);
            ...
            <div className=" ">
                <div className="flex self-center justify-center h-[100vh]  items-center">
                    ...
                    >
                        ...
                        <button className="bg-black disabled:opacity-70 disabled:cursor-not-allowed flex justify-center items-center  transition-all duration-150 gap-x-3 text-white w-full p-4 rounded-2xl font-semibold">
                            <div> VERIFY OTP </div> <ArrowForwardIosIcon className="scale-75  " />
                        </button>
                    </form>
                </div>
            </div>
