...
//     })
// }
export function fetchExamControlModal(campus: string, program:string, semester:number, programtype:string): Promise<QueryResult<any>>{
    return new Promise((resolve, reject) =>{
        pool.query(fetchExamControl,[campus, program, semester, programtype], (error, results)=>{
            if(error) {
                reject(error);
            } else {
                resolve(results);
...
export function categoryLateralModel(rollno:string, name:string,  category:string, is_lateral:string): Promise<QueryResult<any>>{
    return new Promise((resolve, reject) =>{
        pool.query(categoryLateralQuery,[rollno, name, category, is_lateral], (error, results)=>{
            if(error) {
                reject(error);
            } else {
                resolve(results);
